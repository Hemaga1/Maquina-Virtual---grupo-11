...
void LDH(tipoMV *programa, uint32_t op1, uint32_t op2)
{
    ...
    if (tipo_op1 == 3)
    {
        ...
        programa->registros[op1 & 0x1F] = (programa->registros[op1 & 0x1F] & 0xFFFF) + (valor_cargar << 16);
}
...
void LDL(tipoMV *programa, uint32_t op1, uint32_t op2)
{
    ...
    if (tipo_op1 == 3)
    {
        ...
        programa->registros[op1 & 0x1F] = (programa->registros[op1 & 0x1F] & 0xFFFF0000) + valor_cargar;
}
    ...
    if ((op1 & 1) == 1)
    {
        for (int i = 0; i < 5; i++)
        {
            if ((formato & 1) == 1)
                ...
                {
                case 0:
                    scanf("%d", &programa->memoria[posicion]);
                    break;
                case 1:
                    scanf("%c", &programa->memoria[posicion]);
                    break;
                case 2:
                    scanf("%o", &programa->memoria[posicion]);
                    break;
                case 3:
                    scanf("%X", &programa->memoria[posicion]);
                    break;
                case 4:
                    scanf("%d", &programa->memoria[posicion]);
                    break;
    ...
    else
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            if ((formato & 1) == 1)
                ...
                case 4:
                    printf("[%X]: ", posicion);
                    MostrarBinario(programa->registros[MBR]);
                    printf("\n");
                    break;
                }
            formato = formato >> 1;
...
void JZ(tipoMV *programa, uint32_t op1, uint32_t op2)
{
   tipoMV aux = *programa;
    ...
    {
        JMP(programa, op1 & 0x0FFF, op2 & 0x0FFF );
    }
}
